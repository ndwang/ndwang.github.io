...
So I built a mod that monitors for decision points where player input is needed. When it detects one, it serializes the complete game state to JSON: player HP, deck, relics, potions, the full combat board with enemy intents and status effects, map layout, event options, shop inventory. Everything a human player would see, rendered as structured data. The agent polls for state, feeds it to Claude along with a system prompt describing the game's rules, and gets back actions through tool use. It also maintains two knowledge bases, one persistent across runs and one per-run, so the AI can record what it learns and carry insights forward.
...
The gameplay, though? That's where things got hilariously interesting.
...
The AI's reasoning was glorious. It looked at the timer. It read the description. And it concluded: "What will happen when sandpit reaches 0? I do not know. For safety, I will *not* play Frantic Escape and instead observe what happens when sandpit reaches 0." It then ended its turn to "save energy," even though energy doesn't carry over to the next turn anyway.
...
Sonnet, by contrast, wrote things like "I didn't use this card on turn 5, which was a bad decision" and "I used this on turn 3. Perfect decision." These are useless because they're tied to a specific game state that will never recur. Worse, Sonnet would reference specific relics it had, which actually confused Opus in later runs into thinking *it* had those relics too. The gap between the models was real.

### Failure Modes

Across all these runs, the same failure modes kept appearing.

**No intrinsic curiosity.** It never played a card just to see what it would do. Never tested a mechanic it didn't understand. When confused about doom timing, it didn't set up a situation to observe the answer. It either guessed or wasted tokens reasoning in circles.

**No learning from experience.** You might think this is obvious since LLMs are stateless, but I gave the agent persistent knowledge bases specifically to solve this. It had tools to write down observations and read them back in future decisions. It used them, but never for the things that mattered. The card reward loop happened dozens of times in the same run. The doom confusion persisted across turns. The Osty misunderstanding survived across entire games. The agent never wrote "the card reward stays on the reward screen after I skip it" or "doom triggers after the enemy acts, not before." Each decision was made in something close to isolation, informed by training data but barely by lived experience.

**No common sense.** The sandworm fight is the clearest example. You're fighting a giant sandworm in the dunes. It's filling your deck with cards called *Frantic Escape*. There's a countdown timer with an ominous warning. Every piece of context screams danger. A human reads the room. The AI read the text, shrugged, and chose to wait and see.

But both models shared a telling blind spot. They wrote reflections and summaries, never questions and answers. The knowledge base was full of "here's what happened" and empty of "here's what I don't understand and here's how I'll find out." No model ever used it to track its own confusion or systematically test hypotheses. I guess this is a consequence of post-training for coding tasks where summarization is emphasized.
...
There's been a wave of excitement in academia about using AI agents to automate scientific research. Some projects claim that LLM agents can autonomously generate hypotheses, design experiments, and write papers. The vision is compelling: AI that doesn't just assist with science but *does* science, making independent discoveries.

But what does "discovery" actually require? Encountering something you've never seen before and figuring out what it is. Forming hypotheses about mechanisms you don't understand and testing them systematically. Noticing when your mental model doesn't match reality and updating it.

These are exactly the things the AI couldn't do in Slay the Spire 2.

The game is a near-perfect controlled experiment for this question. The mechanics are described in plain English. The feedback loop is immediate — play a card, see what happens. The environment is fully observable. There is no noise, no measurement error, no confounding variables. If an AI can't figure out how Osty works under these conditions, what happens when you point it at a genuinely novel physical phenomenon with noisy data and no English description of the underlying mechanism?

I suspect the reason AI agents look impressive in current scientific applications is that the science isn't actually new to them. If a model has ingested thousands of papers on biology, it can say smart things about biology. That's not discovery, that's retrieval and recombination. The real test is whether an AI could discover something that isn't already, in some form, in its training data.

Put an LLM in 1900 and ask it to discover special relativity from the Michelson-Morley experiment. I don't think it could yet. Not because it lacks intelligence, but because it lacks the thing that drove Einstein to his breakthrough: curiosity, intuitions, imagination.

## What Would It Take?

I don't have an answer. But I think the Slay the Spire experiment helps frame the question.

What the AI is missing isn't more parameters or more training data. It's something more structural: the ability to notice its own confusion and act on it, to poke at things it doesn't understand, to form a mental model and update it when reality disagrees.

I have no doubt that a lot of the problems can be mitigated with human in the loop and better prompting. I could write down how exactly the mechanics work and clear up any ambiguity. I could interrupt the agent's decision loops and guide it in the right direction. But I feel that deviates from the spirit of this experiment. That is human-AI collaboration, not fully autonomous discovery.
...
"Dude I just found out you can play Slay the Spire using human brain, build infinite combos, beat the game, without using any tokens!"